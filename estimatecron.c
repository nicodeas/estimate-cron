...
int pid = 0; // NOTE: also used to keep track of total commands executed
...
int process_month(char monthStr[]) {
  ...
  char months[NUM_MONTHS][4] = {"jan", "feb", "mar", "apr", "may", "jun",
...
int parse_weekdays(char *weekday) {
  ...
  char weekdays[NUM_WEEKDAYS][4] = {"sun", "mon", "tue", "wed",
                                    "thu", "fri", "sat"};
...
void process_estimates(char *filename) {
  ...
  while (fgets(buffer, MAX_CHARS, fp) != NULL) {
    ...
    printf("%s:\t\truns for %i minutes\n", commands[total_commands].name,
           commands[total_commands].minutes_to_complete);
    total_commands++;
  }
  fclose(fp);
...
void process_crontab(char *filename) {
  ...
  while (fgets(buffer, MAX_CHARS, fp) != NULL) {
    ...
    sscanf(buffer, "%s %s %s %s %s %s", minute, hour, day, month, weekday,
           name);
...
void end_processes(time_t current_time) {
  for (int i = 0; i < MAX_ALLOWED_CONCURRENT_PROCESSES; i++) {
    ...
    if (difftime(processes[i].end_time, current_time) == 0 &&
        !processes[i].ended) {
      processes[i].ended = true;
...
void simultate(int month) {
  ...
  while (!simulation_complete) {
    ...
    for (int i = 0; i < total_commands; i++) {
      ...
      if ((commands[i].week_day == -1 || commands[i].week_day == weekday) &&
          (commands[i].day == -1 || commands[i].day == day) &&
          (commands[i].hour == -1 || commands[i].hour == hour) &&
          (commands[i].minute == -1 || commands[i].minute == minute) &&
          (commands[i].month == -1 || commands[i].month == month)) {
...
int main(int argc, char *argv[]) {
  if (argc != 4) {
    printf("Usage: %s takes in exactly 3 arguments: month, crontabfile, "
           "estimatesfile\n",
           argv[0]);
    exit(EXIT_FAILURE);
  }
